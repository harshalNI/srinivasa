...
if (!defined('BASEPATH'))
	...
	class AdmincontrollerList {
		...
		private function setControllers() {
			...
			foreach(glob(APPPATH . 'controllers/cms_admin/*') as $controller) {
				...
				if(is_dir($controller)) {
					...
					if(is_array($aMethods)){
						foreach($aMethods as $method) {
							if (substr($method, 0, 1) !== '_'){
								if($method != '__construct' && $method != 'get_instance' && $method != $controllername && $method != 'Admin404Error') {
									$aUserMethods[] = $method;
								}
							}
					...
					//echo"<pre>";print_r($aUserMethods);exit;
